...
def init_db():
    ...
    c = conn.cursor()
    c.execute('CREATE TABLE IF NOT EXISTS restaurant (id INTEGER PRIMARY KEY AUTOINCREMENT, اسم_الصنف TEXT, السعر TEXT, القسم TEXT)')
    conn.commit()
    conn.close()
...
def home():
    ...
    if request.method == 'POST':
        values = [request.form.get(f, '') for f in ['اسم_الصنف', 'السعر', 'القسم']]
        conn = sqlite3.connect(DB_NAME)
        c = conn.cursor()
        c.execute('INSERT INTO restaurant (اسم_الصنف, السعر, القسم) VALUES (?, ?, ?)', values)
        conn.commit()
        conn.close()
        return redirect('/')
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute('SELECT * FROM restaurant ORDER BY id ASC')
    items = c.fetchall()
    count = len(items)
    conn.close()
    for item in items:
        ...
        rows += '</tr>'
    html = '<!DOCTYPE html><html lang=ar dir=rtl><head><meta charset=UTF-8><title>نظام مطعم</title>' + STYLE + '</head>'
    html += '<body><div class=container>'
    html += '<h1 class=legendary-title>نظام مطعم</h1>'
    html += '<div class=form-box><form method=POST style=display:flex;gap:10px;flex-wrap:wrap;>'
    html += '<input type=text name=اسم_الصنف placeholder=اسم_الصنف required> <input type=text name=السعر placeholder=السعر required> <input type=text name=القسم placeholder=القسم required>'
    ...
    html += '<thead><tr><th>ID</th><th>اسم_الصنف</th><th>السعر</th><th>القسم</th><th>إجراءات</th></tr></thead>'
...
def delete_item(item_id):
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute('DELETE FROM restaurant WHERE id=?', (item_id,))
    conn.commit()
    conn.close()
    return redirect('/')
...
def edit_item(item_id):
    if request.method == 'POST':
        ...
        conn = sqlite3.connect(DB_NAME)
        c = conn.cursor()
        set_parts = ', '.join([f + '=?' for f in ['اسم_الصنف', 'السعر', 'القسم']])
        c.execute('UPDATE restaurant SET ' + set_parts + ' WHERE id=?', values + [item_id])
        conn.commit()
        conn.close()
        return redirect('/')
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute('SELECT * FROM restaurant WHERE id=?', (item_id,))
    item = c.fetchone()
    conn.close()
    if not item:
        return redirect('/')
    edit_inputs = ''
    for i, f in enumerate(['اسم_الصنف', 'السعر', 'القسم']):
...
def export_csv():
    ...
    writer.writerow(['ID'] + ['اسم_الصنف', 'السعر', 'القسم'])
    for item in items:
        writer.writerow(item)
    output.seek(0)
    return output.getvalue(), {'Content-Type': 'text/csv', 'Content-Disposition': 'attachment; filename=restaurant.csv'}
...
def sort_items(column):
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute('SELECT * FROM restaurant ORDER BY ' + column + ' ASC')
    items = c.fetchall()
    count = len(items)
    conn.close()
    rows = ''
    ...
    html += '<table class=legendary-table>'
    html += '<thead><tr><th>ID</th><th>اسم_الصنف</th><th>السعر</th><th>القسم</th><th>اجراءات</th></tr></thead>'
    html += '<tbody>' + rows + '</tbody>'
    html += '</table>'
...
def analyze():
    ...
    if total == 0:
        ...
        for f in ['اسم_الصنف', 'السعر', 'القسم']:
            c.execute('SELECT COUNT(*) FROM restaurant WHERE ' + f + ' IS NULL OR ' + f + ' = ""')
            empty = c.fetchone()[0]
...
def mock_data():
    ...
    for i in range(100):
        ...
        placeholders = ', '.join(['?'] * len(['اسم_الصنف', 'السعر', 'القسم']))
        cols = ', '.join(['اسم_الصنف', 'السعر', 'القسم'])
        c.execute('INSERT INTO restaurant (' + cols + ') VALUES (' + placeholders + ')', values)
    conn.commit()
    conn.close()
    return redirect('/')
...
def report():
    ...
    for f in ['اسم_الصنف', 'السعر', 'القسم']:
        try:
            c.execute('SELECT SUM(CAST(' + f + ' AS REAL)), AVG(CAST(' + f + ' AS REAL)), MAX(CAST(' + f + ' AS REAL)), MIN(CAST(' + f + ' AS REAL)) FROM restaurant')
            row = c.fetchone()
...
def settings():
    ...
    html += '<div style=display:flex;justify-content:center;gap:15px;flex-wrap:wrap;>'
    html += '<span class=summary-bar>اسم النظام: نظام مطعم</span>'
    html += '<span class=summary-bar>الجدول: restaurant</span>'
    html += '<span class=summary-bar>الحقول: 3</span>'
    html += '</div><br><a href=/ class=legendary-btn>رجوع</a></div></body></html>'
    return html
...
def search():
    ...
    html += '<table class=legendary-table><thead><tr><th>ID</th><th>اسم_الصنف</th><th>السعر</th><th>القسم</th><th>اجراءات</th></tr></thead><tbody>' + rows + '</tbody></table>'
    html += '</div></body></html>'
    return html
...
def api_data():
    ...
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute('SELECT * FROM restaurant ORDER BY id ASC LIMIT 100')
    items = c.fetchall()
    conn.close()
    data = []
    for item in items:
...
def api_stats():
    ...
    total = c.fetchone()[0]
    conn.close()
    return json.dumps({'total': total, 'table': 'restaurant', 'fields': ['اسم_الصنف', 'السعر', 'القسم']})
