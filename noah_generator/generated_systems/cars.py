...
def init_db():
    ...
    c = conn.cursor()
    c.execute('CREATE TABLE IF NOT EXISTS cars (id INTEGER PRIMARY KEY AUTOINCREMENT, الماركة TEXT, الموديل TEXT, السعر TEXT)')
    conn.commit()
    conn.close()
...
def home():
    ...
    if request.method == 'POST':
        values = [request.form.get(f, '') for f in ['الماركة', 'الموديل', 'السعر']]
        conn = sqlite3.connect(DB_NAME)
        c = conn.cursor()
        c.execute('INSERT INTO cars (الماركة, الموديل, السعر) VALUES (?, ?, ?)', values)
        conn.commit()
        conn.close()
        return redirect('/')
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute('SELECT * FROM cars ORDER BY id ASC')
    items = c.fetchall()
    count = len(items)
    conn.close()
    for item in items:
        ...
        rows += '</tr>'
    html = '<!DOCTYPE html><html lang=ar dir=rtl><head><meta charset=UTF-8><title>نظام مكاتب السيارات </title>' + STYLE + '</head>'
    html += '<body><div class=container>'
    html += '<h1 class=legendary-title>نظام مكاتب السيارات </h1>'
    html += '<div class=form-box><form method=POST style=display:flex;gap:10px;flex-wrap:wrap;>'
    html += '<input type=text name=الماركة placeholder=الماركة required> <input type=text name=الموديل placeholder=الموديل required> <input type=text name=السعر placeholder=السعر required>'
    ...
    html += '<thead><tr><th>ID</th><th>الماركة</th><th>الموديل</th><th>السعر</th><th>إجراءات</th></tr></thead>'
...
def edit_item(item_id):
    if request.method == 'POST':
        ...
        conn = sqlite3.connect(DB_NAME)
        c = conn.cursor()
        set_parts = ', '.join([f + '=?' for f in ['الماركة', 'الموديل', 'السعر']])
        c.execute('UPDATE cars SET ' + set_parts + ' WHERE id=?', values + [item_id])
        conn.commit()
        conn.close()
        return redirect('/')
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute('SELECT * FROM cars WHERE id=?', (item_id,))
    item = c.fetchone()
    conn.close()
    if not item:
        return redirect('/')
    edit_inputs = ''
    for i, f in enumerate(['الماركة', 'الموديل', 'السعر']):
...
def export_csv():
    ...
    writer.writerow(['ID'] + ['الماركة', 'الموديل', 'السعر'])
    for item in items:
        writer.writerow(item)
    output.seek(0)
    return output.getvalue(), {'Content-Type': 'text/csv', 'Content-Disposition': 'attachment; filename=cars.csv'}
...
def mock_data():
    ...
    for i in range(100):
        values = []
        for f in ['الماركة', 'الموديل', 'السعر']:
        ...
        placeholders = ', '.join(['?'] * len(['الماركة', 'الموديل', 'السعر']))
        cols = ', '.join(['الماركة', 'الموديل', 'السعر'])
        c.execute('INSERT INTO cars (' + cols + ') VALUES (' + placeholders + ')', values)
    conn.commit()
    conn.close()
    return redirect('/')
