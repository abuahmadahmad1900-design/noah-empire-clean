...
def init_db():
    ...
    c = conn.cursor()
    c.execute('CREATE TABLE IF NOT EXISTS hotels (id INTEGER PRIMARY KEY AUTOINCREMENT, اسم_النزيل TEXT, الغرفة TEXT, تاريخ_الوصول TEXT)')
    conn.commit()
    conn.close()
...
def home():
    ...
    if request.method == 'POST':
        values = [request.form.get(f, '') for f in ['اسم_النزيل', 'الغرفة', 'تاريخ_الوصول']]
        conn = sqlite3.connect(DB_NAME)
        c = conn.cursor()
        c.execute('INSERT INTO hotels (اسم_النزيل, الغرفة, تاريخ_الوصول) VALUES (?, ?, ?)', values)
        conn.commit()
        conn.close()
        return redirect('/')
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute('SELECT * FROM hotels ORDER BY id ASC')
    items = c.fetchall()
    count = len(items)
    conn.close()
    for item in items:
        ...
        rows += '</tr>'
    html = '<!DOCTYPE html><html lang=ar dir=rtl><head><meta charset=UTF-8><title>نظام فنادق</title>' + STYLE + '</head>'
    html += '<body><div class=container>'
    html += '<h1 class=legendary-title>نظام فنادق</h1>'
    html += '<div class=form-box><form method=POST style=display:flex;gap:10px;flex-wrap:wrap;>'
    html += '<input type=text name=اسم_النزيل placeholder=اسم_النزيل required> <input type=text name=الغرفة placeholder=الغرفة required> <input type=text name=تاريخ_الوصول placeholder=تاريخ_الوصول required>'
    ...
    html += '<thead><tr><th>ID</th><th>اسم_النزيل</th><th>الغرفة</th><th>تاريخ_الوصول</th><th>إجراءات</th></tr></thead>'
...
def delete_item(item_id):
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute('DELETE FROM hotels WHERE id=?', (item_id,))
    conn.commit()
    conn.close()
    return redirect('/')
...
def edit_item(item_id):
    if request.method == 'POST':
        ...
        conn = sqlite3.connect(DB_NAME)
        c = conn.cursor()
        set_parts = ', '.join([f + '=?' for f in ['اسم_النزيل', 'الغرفة', 'تاريخ_الوصول']])
        c.execute('UPDATE hotels SET ' + set_parts + ' WHERE id=?', values + [item_id])
        conn.commit()
        conn.close()
        return redirect('/')
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute('SELECT * FROM hotels WHERE id=?', (item_id,))
    item = c.fetchone()
    conn.close()
    if not item:
        return redirect('/')
    edit_inputs = ''
    for i, f in enumerate(['اسم_النزيل', 'الغرفة', 'تاريخ_الوصول']):
...
def export_csv():
    ...
    writer.writerow(['ID'] + ['اسم_النزيل', 'الغرفة', 'تاريخ_الوصول'])
    for item in items:
        writer.writerow(item)
    output.seek(0)
    return output.getvalue(), {'Content-Type': 'text/csv', 'Content-Disposition': 'attachment; filename=hotels.csv'}
...
def mock_data():
    ...
    for i in range(100):
        values = []
        for f in ['اسم_النزيل', 'الغرفة', 'تاريخ_الوصول']:
        ...
        placeholders = ', '.join(['?'] * len(['اسم_النزيل', 'الغرفة', 'تاريخ_الوصول']))
        cols = ', '.join(['اسم_النزيل', 'الغرفة', 'تاريخ_الوصول'])
        c.execute('INSERT INTO hotels (' + cols + ') VALUES (' + placeholders + ')', values)
    conn.commit()
    conn.close()
    return redirect('/')
