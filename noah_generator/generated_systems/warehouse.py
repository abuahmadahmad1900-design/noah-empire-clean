...
def init_db():
    ...
    c = conn.cursor()
    c.execute('CREATE TABLE IF NOT EXISTS warehouse (id INTEGER PRIMARY KEY AUTOINCREMENT, اسم_الصنف TEXT, الكمية TEXT, المورد TEXT)')
    conn.commit()
    conn.close()
...
def home():
    ...
    if request.method == 'POST':
        values = [request.form.get(f, '') for f in ['اسم_الصنف', 'الكمية', 'المورد']]
        conn = sqlite3.connect(DB_NAME)
        c = conn.cursor()
        c.execute('INSERT INTO warehouse (اسم_الصنف, الكمية, المورد) VALUES (?, ?, ?)', values)
        conn.commit()
        conn.close()
        return redirect('/')
    conn = sqlite3.connect(DB_NAME)
    ...
    for item in items:
        ...
        rows += '</tr>'
    html = '<!DOCTYPE html><html lang=ar dir=rtl><head><meta charset=UTF-8><title>نظام مخزن</title>' + STYLE + '</head>'
    html += '<body><div class=container>'
    html += '<h1 class=legendary-title>نظام مخزن</h1>'
    html += '<div class=form-box><form method=POST style=display:flex;gap:10px;flex-wrap:wrap;>'
    html += '<input type=text name=اسم_الصنف placeholder=اسم_الصنف required> <input type=text name=الكمية placeholder=الكمية required> <input type=text name=المورد placeholder=المورد required>'
    ...
    html += '<thead><tr><th>ID</th><th>اسم_الصنف</th><th>الكمية</th><th>المورد</th><th>إجراءات</th></tr></thead>'
...
def delete_item(item_id):
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute('DELETE FROM warehouse WHERE id=?', (item_id,))
    conn.commit()
    conn.close()
    return redirect('/')
...
def edit_item(item_id):
    if request.method == 'POST':
        ...
        conn = sqlite3.connect(DB_NAME)
        c = conn.cursor()
        set_parts = ', '.join([f + '=?' for f in ['اسم_الصنف', 'الكمية', 'المورد']])
        c.execute('UPDATE warehouse SET ' + set_parts + ' WHERE id=?', values + [item_id])
        conn.commit()
        conn.close()
        return redirect('/')
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute('SELECT * FROM warehouse WHERE id=?', (item_id,))
    item = c.fetchone()
    conn.close()
    if not item:
        return redirect('/')
    edit_inputs = ''
    for i, f in enumerate(['اسم_الصنف', 'الكمية', 'المورد']):
...
def export_csv():
    ...
    writer.writerow(['ID'] + ['اسم_الصنف', 'الكمية', 'المورد'])
    for item in items:
        writer.writerow(item)
    output.seek(0)
    return output.getvalue(), {'Content-Type': 'text/csv', 'Content-Disposition': 'attachment; filename=warehouse.csv'}
...
def sort_items(column):
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute('SELECT * FROM warehouse ORDER BY ' + column + ' ASC')
    items = c.fetchall()
    count = len(items)
    conn.close()
    rows = ''
    ...
    html += '<table class=legendary-table>'
    html += '<thead><tr><th>ID</th><th>اسم_الصنف</th><th>الكمية</th><th>المورد</th><th>اجراءات</th></tr></thead>'
    html += '<tbody>' + rows + '</tbody>'
    html += '</table>'
...
def analyze():
    ...
    if total == 0:
        ...
        for f in ['اسم_الصنف', 'الكمية', 'المورد']:
            c.execute('SELECT COUNT(*) FROM warehouse WHERE ' + f + ' IS NULL OR ' + f + ' = ""')
            empty = c.fetchone()[0]
...
def mock_data():
    ...
    for i in range(100):
        ...
        placeholders = ', '.join(['?'] * len(['اسم_الصنف', 'الكمية', 'المورد']))
        cols = ', '.join(['اسم_الصنف', 'الكمية', 'المورد'])
        c.execute('INSERT INTO warehouse (' + cols + ') VALUES (' + placeholders + ')', values)
    conn.commit()
    conn.close()
    return redirect('/')
...
def report():
    ...
    for f in ['اسم_الصنف', 'الكمية', 'المورد']:
        try:
            c.execute('SELECT SUM(CAST(' + f + ' AS REAL)), AVG(CAST(' + f + ' AS REAL)), MAX(CAST(' + f + ' AS REAL)), MIN(CAST(' + f + ' AS REAL)) FROM warehouse')
            row = c.fetchone()
...
def settings():
    ...
    html += '<div style=display:flex;justify-content:center;gap:15px;flex-wrap:wrap;>'
    html += '<span class=summary-bar>اسم النظام: نظام مخزن</span>'
    html += '<span class=summary-bar>الجدول: warehouse</span>'
    html += '<span class=summary-bar>الحقول: 3</span>'
    html += '</div><br><a href=/ class=legendary-btn>رجوع</a></div></body></html>'
    return html
...
def search():
    ...
    html += '<table class=legendary-table><thead><tr><th>ID</th><th>اسم_الصنف</th><th>الكمية</th><th>المورد</th><th>اجراءات</th></tr></thead><tbody>' + rows + '</tbody></table>'
    html += '</div></body></html>'
    return html
...
def api_data():
    ...
    conn = sqlite3.connect(DB_NAME)
    c = conn.cursor()
    c.execute('SELECT * FROM warehouse ORDER BY id ASC LIMIT 100')
    items = c.fetchall()
    conn.close()
    data = []
    for item in items:
...
def api_stats():
    ...
    total = c.fetchone()[0]
    conn.close()
    return json.dumps({'total': total, 'table': 'warehouse', 'fields': ['اسم_الصنف', 'الكمية', 'المورد']})
